...
mkdir -p "$TEST_TMP/bin" "$TEST_TMP/libexec/rtide"
git -C "$TEST_TMP" init -q
printf '#!/usr/bin/env bash\n' > "$TEST_TMP/bin/rtide"
printf '#!/usr/bin/env bash\n' > "$TEST_TMP/libexec/rtide/picker.sh"
...
grep -F 'runtime dependency is untracked: libexec/rtide/picker.sh' "$TEST_TMP/err" >/dev/null \
  || fail 'missing runtime dependency was not identified'
git -C "$TEST_TMP" add libexec/rtide/picker.sh
